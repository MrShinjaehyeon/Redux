...
export const createPromiseThunk = (type, promiseCreator) => {
  ...
  // 만약 여러 종류의 파라미터를 전달해야하는 상황에서는 객체 타입의 파라미터를 받아 사용.
...
export const reducerUtils = {
  // 초기 상태. 초기 data 값은 기본적으로 null 이지만 바꿀 수 있음
  ...
  // 따로 값을 지정하면 null로 바꾸지 않고 다른 값을 유지할 수 있음.
...
// 개별 post를 로딩하고 뒤로 간 후 다시 읽었던 포스트를 불러 올 때 리렌더링 하지 않는 방법

// 특정 id 를 처리하는 Thunk 생성함수
const defaultIdSelector = (param) => param; // defaultIdSelector(id){return id} -> id를 그대로 반환하는 function을 변수화함
